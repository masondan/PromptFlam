...
**Core Philosophy**: No barriers to entry. One Room ID per training session. Team-based collaboration. Trainer-controlled access. Simple, minimalist, clean UI and UX
...
- **NewsLab ID**: Alphanumeric string (e.g., `nigeria-0125`). Provided by trainer. Unique per training session. Single gate for all journalists.
- **Trainer ID**: Separate (e.g., `trainer-admin`). Grants trainer-only access.
...
- **Entry**: Via Trainer ID at gatekeeper.
...
- **Permissions**:
   ...
   - Reset NewsLab ID / Trainer ID
   - "Clear NewsLab" (delete all data at end of training)
...
CREATE TABLE newslabs (
  ...
  trainer_id TEXT NOT NULL,       -- e.g., "trainer-admin"
...
1. **Gatekeeper**: Input Trainer ID → Granted access
...
3. **Settings** (`/trainer/settings`):
   ...
   - "Clear NewsLab" button → Confirm → Delete all data (newslabs, teams, stories, journalists)
...
**Deliverables**:
- [ ] NewsLab Entry page: NewsLab ID or Trainer ID, Personal Name
...
- [ ] Trainer ID grants `/trainer/*` access
...
- [ ] Trainer Settings page (`/trainer/settings`):
   - [ ] Display current NewsLab ID, Trainer ID
   - [ ] Reset NewsLab ID button → Generate new ID, update Supabase
   - [ ] Reset Trainer ID button → Generate new ID
...
- [ ] Reset Trainer ID: Old ID no longer works
...
- [ ] Trainer admin: Reset Trainer ID → Clear NewsLab
...
| IDs | NewsLab ID for journalists, Trainer ID for trainers (no passwords) | Matches "zero-login" philosophy, simple UX |
